        ...
        try:
            ...
            for delimiter in delimiters:
                ...
                if column_counts:
                    ...
                    # 如果所有行的列数都相同且大于1，这是理想的分隔符
                    if all(count == column_counts[0] for count in column_counts) and column_counts[0] > 1:
                        best_delimiter = delimiter
                        max_columns = column_counts[0]
                        break
                    # 否则选择平均列数最多的分隔符
                    elif avg_columns > max_columns:
                        best_delimiter = delimiter
                        max_columns = avg_columns
            
            if not best_delimiter or max_columns < 2:
                # 如果自动识别失败，尝试使用CSV格式解析
                try:
                    with open(filename, 'r', encoding='utf-8') as f:
                        reader = csv.DictReader(f)
                        return list(reader)
            ...
            for i, line in enumerate(lines):
                ...
                if i == 0:
                    ...
                    if header_line:
                        # 如果有表头，创建字典
                        row_data = {}
                        for j, header in enumerate(header_line):
                            if j < len(columns):
                        ...
                        for j, value in enumerate(columns):
                            row_data[f'column_{j+1}'] = value
                        data.append(row_data)
            
            # 如果没有表头但第一行看起来不是表头（包含数字等），则将其也作为数据
            if not header_line and lines:
                first_line = [col.strip() for col in lines[0].strip().split(best_delimiter)]
                if any(any(char.isdigit() for char in col) for col in first_line):
                    row_data = {}
                    for j, value in enumerate(first_line):
                        row_data[f'column_{j+1}'] = value
