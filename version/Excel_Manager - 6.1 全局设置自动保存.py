# [文件名]: Excel_Manager - 6.1 多线程优化版.py
...
# 检查并安装 pypinyin（仅检查可用性，不重复导入）
try:
    lazy_pinyin  # 已在上方导入，仅验证可用性
except NameError:
...
class MainWindow(QMainWindow):
    ...
    def load_data(self):
        ...
        try:
            ...
            # 创建并启动数据加载线程
            self.load_worker = LoadDataWorker(self.user_db, self.columns_config)
    ...
    def _on_data_loaded(self, result):
        """数据加载完成的回调（主线程）"""
        self._cleanup_worker(self.load_worker)
        try:
            data = result['data']
    ...
    def search_all_columns(self):
        ...
        try:
            ...
            # 使用多线程执行搜索
            self.search_worker = SearchDataWorker(self.user_db, keyword)
    ...
    def export_data(self):
        ...
        try:
            ...
            # 使用多线程导出
            self.export_worker = ExportDataWorker(self.user_db, filename)
    ...
    def manual_backup(self):
        ...
        self.backup_worker = BackupWorker(self.user_db, backup_type="manual")
    ...
    def cleanup_duplicate_data(self):
        ...
        try:
            ...
            preview_btn.clicked.connect(lambda: self.preview_duplicates(selected_fields))
            button_layout.addWidget(preview_btn)
            
            button_layout.addStretch()
            
            start_btn = QPushButton('🚀 开始清理')
            start_btn.clicked.connect(lambda: self.execute_duplicate_cleanup(
                selected_fields, 
