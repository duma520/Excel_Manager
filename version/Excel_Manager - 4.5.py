...
class MainWindow(QMainWindow):
    ...
    def show_item_detail(self, index):
        ...
        for col_idx, col in enumerate(self.columns_config):
            if col_idx < len(values):
                ...
                # 添加防御性检查
                if value is not None:  # 确保对象存在
                    value.setTextInteractionFlags(Qt.TextSelectableByMouse)
                
                # 高亮重要字段
                if col.get('is_unique', False):
                    label.setStyleSheet("color: #4CAF50; font-weight: bold;")  # 绿色
                if col.get('is_required', False):
                    label.setStyleSheet("color: #F44336; font-weight: bold;")  # 红色
        ...
        if barcode_cols:
            ...
            for col in barcode_cols:
                ...
                if col_idx < len(values) and values[col_idx]:
                    ...
                    is_valid, error_msg = self.validate_barcode(value)
                    
                    if not is_valid:
                        # 显示错误信息
                        error_label = QLabel(f"⚠️ 无效条码: {error_msg}")
                        error_label.setStyleSheet("color: red; font-weight: bold;")
                        barcode_layout.addWidget(error_label)
                        continue
                    ...
                    if barcode_img:
                        ...
                        if not str(value).isascii():
                            tip = QLabel(f"⚠️ 注意：'{col['label']}'包含非ASCII字符，已自动编码")
                            tip.setStyleSheet("color: #FF9800; font-style: italic;")
                            barcode_layout.addWidget(tip)
                        ...
                        # 条形码图像
                        img_label = QLabel()
                        img_label.setPixmap(QPixmap.fromImage(barcode_img))
            ...
            for col in qrcode_cols:
                ...
                if col_idx < len(values) and values[col_idx]:
                    ...
                    if qrcode_img:
                        ...
                        # 二维码图像
                        img_label = QLabel()
                        img_label.setPixmap(QPixmap.fromImage(qrcode_img))
                        img_label.setAlignment(Qt.AlignCenter)
    ...
    def validate_barcode(self, barcode_value):
        ...
        if len(barcode_value) < 6:
            return False, "条码长度不能少于6位"
        
        # 检查是否只包含有效字符
        if not barcode_value.isalnum():
            return False, "条码只能包含字母和数字"
