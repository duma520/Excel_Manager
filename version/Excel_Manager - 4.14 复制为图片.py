...
class MainWindow(QMainWindow):
    ...
    def show_context_menu(self, position):
        ...
        # 复制为图片
        copy_as_image_action = QAction('复制为图片', self)
    ...
    def copy_selected_as_image(self):
        ...
        try:
            ...
            # 计算要渲染的区域大小
            width = 0
            height = self.data_table.horizontalHeader().height()  # 包含表头高度
            
            for col in cols:
                width += self.data_table.columnWidth(col)
            for row in rows:
                height += self.data_table.rowHeight(row)
            ...
            for col in cols:
                ...
                if header:
                    ...
                    painter.fillRect(header_rect, QColor('#F5F5F5'))  # 表头背景色
            ...
            for row in rows:
                ...
                for col in cols:
                    ...
                    if item:
                        text = item.text()
                        text_rect = cell_rect.adjusted(5, 0, -5, 0)  # 左右边距
