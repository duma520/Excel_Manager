    ...
    def read_import_file(self, filename, import_config):
        ...
        if file_type in ['csv', 'ansi']:
            ...
            if file_size > 50 * 1024 * 1024:  # 50MB
                ...
                try:
                    # 使用生成器读取，但限制最大行数避免内存问题
                    data = []
                    max_rows = import_config.get('max_rows', 100000)  # 默认最多10万行
                    for i, record in enumerate(self.read_large_csv_file_generator(filename, import_config)):
                        if i >= max_rows:
                            print(f"[DEBUG] 达到最大行数限制 {max_rows}，停止读取")
                            break
                        data.append(record)
                    
                    if not data:
                        print("[DEBUG] 生成器读取返回空数据，尝试常规读取方法")
                        # 如果生成器返回空，回退到常规读取
    ...
    def read_large_csv_file_generator(self, filename, import_config):
        ...
        try:
            ...
            with open(filename, 'rb') as f:
                ...
                encoding_result = chardet.detect(raw_data)
                file_encoding = encoding_result.get('encoding', 'utf-8')
                print(f"[DEBUG] 大文件检测到编码: {file_encoding}")
            ...
            with open(filename, 'r', encoding=file_encoding, errors='replace') as f:
                ...
                for line in f:
                    ...
                    if line_count % 10000 == 0:
                        print(f"[DEBUG] 已读取 {line_count} 行数据")
                    
                    yield record
                
                print(f"[DEBUG] 大文件总共读取 {line_count} 行数据")
