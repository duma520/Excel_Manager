...
class LoginWindow(QMainWindow):
    ...
    def read_import_file(self, filename, import_config):
        ...
        if file_type in ['csv', 'ansi']:  # 添加 ansi 支持
            # 根据文件大小选择读取方式
    ...
    def batch_ultra_fast_import_enhanced(self):
        ...
        unattended_mode.setChecked(True)  # 默认开启
        ...
        if unattended_radio.isChecked():
            ...
            self.execute_unattended_batch_import(
                filenames, 
            ...
            self.batch_ultra_fast_import_original(filenames)
...
class MainWindow(QMainWindow):
    ...
    def execute_ultra_fast_import(self, import_config):
        ...
        try:
            print(f"[DEBUG] 开始导入文件: {filename}")
            print(f"[DEBUG] 字段映射: {mapping}")
            print(f"[DEBUG] 冲突解决策略: {conflict_resolution}")
            ...
            print(f"[DEBUG] 成功读取 {len(data)} 行数据")

            # 2. 数据预处理（可选）
            if preprocess:
                data = self.preprocess_import_data(data, mapping)
                print(f"[DEBUG] 预处理后数据行数: {len(data)}")
    ...
    def batch_ultra_fast_import_enhanced(self):
        ...
        mode_dialog = QDialog(self)
        mode_dialog.setWindowTitle('选择导入模式')
        mode_dialog.resize(400, 250)  # 增加高度以容纳新选项
