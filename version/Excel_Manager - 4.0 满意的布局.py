...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, 
                            ...
                            QToolBar, QSizePolicy, QSpacerItem, QSplitter)
...
class MainWindow(QMainWindow):
    ...
    def show_item_detail(self, index):
        ...
        barcode_col = next((col for col in self.columns_config if col.get('is_barcode', False)), None)
        if barcode_col:
            barcode_idx = self.columns_config.index(barcode_col)
            if barcode_idx < len(values) and values[barcode_idx]:
                barcode_value = values[barcode_idx]
                barcode_img = self.generate_barcode(barcode_value)
                if barcode_img:
                    label = QLabel('条形码:')
        ...
        qrcode_col = next((col for col in self.columns_config if col.get('is_qrcode', False)), None)
        if qrcode_col:
            qrcode_idx = self.columns_config.index(qrcode_col)
            if qrcode_idx < len(values) and values[qrcode_idx]:
                qrcode_value = values[qrcode_idx]
                qrcode_img = self.generate_qrcode(qrcode_value)
                if qrcode_img:
                    label = QLabel('二维码:')
                    self.detail_layout.addWidget(label)
