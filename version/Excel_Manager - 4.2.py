    ...
    def show_item_detail(self, index):
        ...
        for col_idx, col in enumerate(self.columns_config):
            if col_idx < len(values):
                ...
                value = QLabel(str(values[col_idx]))
                value.setTextInteractionFlags(Qt.TextSelectableByMouse)
                
                # 如果是唯一或必填列，添加特殊标记
                if col.get('is_unique', False):
                    label.setText(f"{col['label']} (唯一)")
                    label.setStyleSheet("color: #4CAF50;")  # 绿色
                if col.get('is_required', False):
                    label.setText(f"{col['label']} (必填)")
                    label.setStyleSheet("color: #F44336;")  # 红色
        ...
        if barcode_cols:
            ...
            for col in barcode_cols:
                ...
                if col_idx < len(values) and values[col_idx]:
                    ...
                    if barcode_img:
                        # 添加标签
                        label = QLabel(f'{col["label"]}: {barcode_value}')
                        label.setStyleSheet("font-weight: bold;")
                        barcode_layout.addWidget(label)
        ...
        if qrcode_cols:
            ...
            for col in qrcode_cols:
                ...
                if col_idx < len(values) and values[col_idx]:
                    ...
                    if qrcode_img:
                        # 添加标签
                        label = QLabel(f'{col["label"]}: {qrcode_value}')
                        label.setStyleSheet("font-weight: bold;")
                        qrcode_layout.addWidget(label)
            ...
            if qrcode_layout.count() > 0:
                ...
                scroll_layout.addWidget(qrcode_group)
        
        # 添加操作按钮
        btn_group = QGroupBox()
        btn_layout = QHBoxLayout()
        
        print_btn = QPushButton('打印详情')
        print_btn.clicked.connect(lambda: self.print_item_detail(data))
        btn_layout.addWidget(print_btn)
        
        copy_btn = QPushButton('复制所有信息')
