...
class UserSetupDialog(QDialog):
    ...
    def update_columns_config(self):
        ...
        for i in range(columns_count):
            ...
            # 添加状态显示标签
            status_label = QLabel("当前状态: 普通列")
            status_label.setStyleSheet("color: gray")
            layout.addRow(QLabel("状态:"), status_label)
            
            # 更新状态显示的槽函数
            def update_status():
                status = []
                if barcode_check.isChecked():
                    status.append("一维码列")
                if qrcode_check.isChecked():
                    status.append("二维码列")
                if unique_check.isChecked():
                    status.append("唯一列")
                ...
                status_label.setText(f"当前状态: {status_text}")
            
            # 连接信号
            barcode_check.stateChanged.connect(update_status)
            qrcode_check.stateChanged.connect(update_status)
            unique_check.stateChanged.connect(update_status)
...
class MainWindow(QMainWindow):
    ...
    def load_data(self, search_term=None):
        ...
        # 计算总列数：数据列 + 一维码列(如果有) + 二维码列(如果有)
        total_columns = len(self.column_names)
        if self.barcode_col is not None:
            total_columns += 1
        if self.qrcode_col is not None:
            total_columns += 1
        ...
        if self.barcode_col is not None:
            headers.insert(self.barcode_col, "一维码")
        if self.qrcode_col is not None:
            headers.insert(self.qrcode_col, "二维码")
        ...
        for row_idx, row in enumerate(self.current_data):
            ...
            for col_idx in range(total_columns):
                # 判断当前列是数据列还是条码列
                if (self.barcode_col is not None and col_idx == self.barcode_col) or \
                (self.qrcode_col is not None and col_idx == self.qrcode_col):
                    # 这是条码列
                    if col_idx == self.barcode_col and self.barcode_col is not None:
                        # 一维码列
                        barcode_text = row[self.barcode_col]
                        if barcode_text:
                            img = self.generate_barcode_thumbnail(barcode_text)
                            if img:
                                # 将PIL图像转换为QPixmap
                    ...
                    elif col_idx == self.qrcode_col and self.qrcode_col is not None:
                        # 二维码列
                        qrcode_text = row[self.qrcode_col]
                        if qrcode_text:
                            img = self.generate_qrcode_thumbnail(qrcode_text)
                            if img:
                                # 将PIL图像转换为QPixmap
                                data = img.tobytes("raw", "RGB")
                                qimage = QImage(data, img.size[0], img.size[1], QImage.Format_RGB888)
                ...
                # 普通数据列
                if data_col < len(self.column_names):
                    item = QTableWidgetItem(str(row[data_col+1] if data_col+1 < len(row) else ""))
    ...
    def generate_barcode_thumbnail(self, text):
        """生成一维码缩略图"""
        try:
            barcode_class = barcode.get_barcode_class('code128')
            barcode_img = barcode_class(text, writer=ImageWriter())
            ...
            img.thumbnail((100, 50))  # 缩略图大小
            return img
        except:
            return None

    def generate_qrcode_thumbnail(self, text):
        """生成二维码缩略图"""
        try:
            qr = qrcode.QRCode(
                version=1,
                error_correction=qrcode.constants.ERROR_CORRECT_L,
                box_size=3,  # 更小的box_size以适应缩略图
                border=1,
            )
            qr.add_data(text)
            qr.make(fit=True)
