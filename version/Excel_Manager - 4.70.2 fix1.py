# [文件名]: Excel_Manager - 4.69.1 升级pyside6后.py
...
from PySide6.QtCore import Qt, QSize, QTimer, QRect, QDate
...
class UserDatabase:
    ...
    def _create_indexes(self):
        ...
        try:
            ...
            # 为rowid创建索引（通常SQLite会自动处理）
            self.cursor.execute('CREATE INDEX IF NOT EXISTS idx_rowid ON data(rowid)')
    ...
    def _safe_create_indexes(self):
        ...
        try:
            ...
            # 为rowid创建索引（rowid是SQLite的隐含列，不需要引号）
            try:
                self.cursor.execute('CREATE INDEX IF NOT EXISTS idx_rowid ON data(rowid)')
            except Exception as e:
                print(f"[DEBUG] 创建rowid索引失败: {str(e)}")
