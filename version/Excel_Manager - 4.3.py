...
class MainWindow(QMainWindow):
    ...
    def show_item_detail(self, index):
        ...
        for i in reversed(range(self.detail_layout.count())): 
            self.detail_layout.itemAt(i).widget().setParent(None) if self.detail_layout.itemAt(i).widget() else None
        ...
        for col_idx, col in enumerate(self.columns_config):
            if col_idx < len(values):
                ...
                value = QLabel(str(values[col_idx]) if values[col_idx] else QLabel("(空)"))
